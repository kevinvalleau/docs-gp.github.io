...
#docker run -it --rm -e "JEKYLL_ENV=docker" --volume="$(pwd)":/srv/jekyll --volume="$(pwd)/vendor/bundle:/usr/local/bundle" -p 4000:4000  jekyll/jekyll:4.0 jekyll serve --watch --incremental --force_polling --config  _config_local.yml,_config.docker.yml